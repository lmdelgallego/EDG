(function($) {
	...
	$('audio[controls]').before(function(){
		...
		var data_toggle = document.createElement('a');
			$(data_toggle).html('<i class="fa fa-reorder"></i>');
		...
		var addPlay = function() {
			var play = document.createElement('button');
				$(play).addClass('btn btn-play disabled col-md-1 col-xs-2');
		...
		var addSeek = function() {
			...
			seek.progress = function () {
				...
				var bg = 'rgba(46, 206, 113, 1) 0%';
				bg += ', rgba(46, 206, 113, 1) ' + ((song.currentTime/song.duration) * 100) + '%';
				bg += ', rgba(46, 206, 113, 1) ' + ((song.currentTime/song.duration) * 100) + '%';
				for (var i=0; i<song.buffered.length; i++){
					if (song.buffered.end(i) > song.currentTime && isNaN(song.buffered.end(i)) === false && isNaN(song.buffered.start(i)) === false){
						...
						bg += ', rgba(169, 226, 193, 0) ' + bufferedstart + '%';
						bg += ', rgba(169, 226, 193, 1) ' + bufferedstart + '%';
						bg += ', rgba(169, 226, 193, 1) ' + bufferedend + '%';
						bg += ', rgba(169, 226, 193, 0) ' + bufferedend + '%';
					}
				}
				...
				$(seek).css('background-color', '#2ecc71');
			};
			seek.set = function () {
			...
			var seek_wrapper = document.createElement('div');
				$(seek_wrapper).addClass('btn progress disabled col-md-4 col-xs-10');
		...
		var addTime = function() {
			var time = document.createElement('a');
				$(time).addClass('btn time col-md-3');
		...
		var addMute = function() {
			var mute = document.createElement('button');
				$(mute).addClass('btn btn-vol col-md-1');
		...
		var addVolume = function() {
			...
			var vol_wrapper = document.createElement('div');
				$(vol_wrapper).addClass('btn disabled col-md-3 col-xs-7');
